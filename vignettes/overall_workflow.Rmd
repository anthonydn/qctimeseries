...
* One **time column** (default name: `"DateTime"`), ideally in **POSIXct**.
* One or more **numeric** variables to QC (each will get a `*_qcflag` column).

If your time column isn’t POSIXct, the app will attempt to convert it for display.
...
`qc_window_app()` opens an interactive graphical environment in your browser. Use the **Hide flagged** checkbox to hide flagged points; use **Home zoom** to restore the full window range; optionally pick a **Secondary** variable to overlay (read‑only).

```{r, eval=FALSE}
# Change y_col to the variable you are about to check
sat_site_qc <- qc_window_app(
  sat_site_qc,
...
* **My time column isn’t POSIXct—does it matter?** The app attempts to convert; you can also convert manually with `as.POSIXct(…, tz = "UTC")` before starting.
* **What about variables with many NAs?** The default x‑range respects the configured window width; the y‑axis scales to the data in view.
